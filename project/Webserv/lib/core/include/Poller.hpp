...
public:
    ...
    auto updateEvents(int fd, core::EventType abstract_events) const -> std::expected<void, std::system_error>;

    auto registerCallback(int fd, core::EventType abstract_events, std::function<void()> cb) -> std::expected<void, std::system_error>;
